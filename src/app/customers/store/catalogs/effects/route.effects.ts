...
export class CatalogRouteEffects {
  ...
  createCatalogSuccess: Observable<Action> = this.actions$
    .pipe(
      ofType<ActionWithPayload>(catalogActions.CREATE_SUCCESS),
  ...
  deleteCatalogSuccess: Observable<Action> = this.actions$
    .pipe(
      ofType<ActionWithPayload>(catalogActions.DELETE_SUCCESS),
      map(action => action.payload),
      tap(payload => this.router.navigate(['../../../'], { relativeTo: payload.activatedRoute }))
    );
  ...
  updateFieldSuccess: Observable<Action> = this.actions$
    .pipe(
      ofType<ActionWithPayload>(catalogActions.UPDATE_FIELD_SUCCESS),
      map(action => action.payload),
      tap(payload => this.router.navigate(['../'], { relativeTo: payload.activatedRoute }))
    );
  ...
  deleteFieldSuccess: Observable<Action> = this.actions$
    .pipe(
      ofType<ActionWithPayload>(catalogActions.DELETE_FIELD_SUCCESS),
      map(action => action.payload),
      tap(payload => this.router.navigate(['../../../../../../'], { relativeTo: payload.activatedRoute }))
    );
